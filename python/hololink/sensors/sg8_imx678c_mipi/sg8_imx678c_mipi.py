...
from . import li_i2c_expander, sg8_imx678c_mipi_mode

# Camera info
DRIVER_NAME = "SG3_IMX900C_MIPI"
VERSION = 1.0
...
class Imx678Cam:
    def __init__(
        ...
        self._mode = sg8_imx678c_mipi_mode.Sensor_Mode.Unknown
    ...
    def start(self):
        ...
        for i2c_addr, reg, val in sg8_imx678c_mipi_mode.sensor_start:
            if i2c_addr == sg8_imx678c_mipi_mode.SENSOR_TABLE_WAIT_MS:
    ...
    def stop(self):
        """Stop Streaming"""
        for i2c_addr, reg, val in sg8_imx678c_mipi_mode.sensor_stop:
            if i2c_addr == sg8_imx678c_mipi_mode.SENSOR_TABLE_WAIT_MS:
                time.sleep(val / 1000)  # the val is in ms
            else:
                self.set_register(i2c_addr, reg, val)
    ...
    def configure_camera(self, sensor_mode_set):
        ...
        if (
            sensor_mode_set.value
            == sg8_imx678c_mipi_mode.Sensor_Mode.SENSOR_MODE_3856X2180_30FPS.value
        ):
            sensor_mode_list = sg8_imx678c_mipi_mode.sensor_mode_3856x2180_30fps
        else:
    ...
    def set_exposure_reg_shs0(self, value=0x0C):
        if value < 0x00:
        ...
        self.set_register(sg8_imx678c_mipi_mode.SENSOR_I2C_ADDRESS, sg8_imx678c_mipi_mode.REG_EXP_SHS0_ADDR_MSB, (value >> 16) & 0xFF)
        self.set_register(sg8_imx678c_mipi_mode.SENSOR_I2C_ADDRESS, sg8_imx678c_mipi_mode.REG_EXP_SHS0_ADDR_MID, (value >> 8) & 0xFF)
        self.set_register(sg8_imx678c_mipi_mode.SENSOR_I2C_ADDRESS, sg8_imx678c_mipi_mode.REG_EXP_SHS0_ADDR_LSB, value & 0xFF)
        time.sleep(sg8_imx678c_mipi_mode.SENSOR_WAIT_MS / 1000)
    ...
    def set_exposure_reg_shs1(self, value=0x0C):
        ...
        self.set_register(sg8_imx678c_mipi_mode.SENSOR_I2C_ADDRESS, sg8_imx678c_mipi_mode.REG_EXP_SHS1_ADDR_MSB, (value >> 16) & 0xFF)
        self.set_register(sg8_imx678c_mipi_mode.SENSOR_I2C_ADDRESS, sg8_imx678c_mipi_mode.REG_EXP_SHS1_ADDR_MID, (value >> 8) & 0xFF)
        self.set_register(sg8_imx678c_mipi_mode.SENSOR_I2C_ADDRESS, sg8_imx678c_mipi_mode.REG_EXP_SHS1_ADDR_LSB, value & 0xFF)
        time.sleep(sg8_imx678c_mipi_mode.SENSOR_WAIT_MS / 1000)
    ...
    def set_analog_gain_reg(self, value=0x0C):
        ...
        self.set_register(sg8_imx678c_mipi_mode.SENSOR_I2C_ADDRESS, sg8_imx678c_mipi_mode.REG_AG_LSB, (value >> 8) & 0xFF)
        self.set_register(sg8_imx678c_mipi_mode.SENSOR_I2C_ADDRESS, sg8_imx678c_mipi_mode.REG_AG_MSB, value & 0xFF)
        time.sleep(sg8_imx678c_mipi_mode.SENSOR_WAIT_MS / 1000)

    def set_mode(self, sensor_mode_set):
        if sensor_mode_set.value < len(sg8_imx678c_mipi_mode.Sensor_Mode):
            self._mode = sensor_mode_set
            mode = sg8_imx678c_mipi_mode.sensor_frame_format[self._mode.value]
    ...
    def configure_converter(self, converter):
        ...
        if self._pixel_format == hololink_module.sensors.csi.PixelFormat.RAW_12:
            ...
            start_byte += received_line_bytes * 20
        else:
            raise Exception(f"Incorrect pixel format={self._pixel_format} for IMX577.")
        converter.configure(
            start_byte,
            received_line_bytes,
            self._width,
